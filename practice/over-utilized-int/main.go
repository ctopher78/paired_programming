...
func main() {
	...
	regx := regexp.MustCompile(`(?s)^(\w+\d/\d).*BW (\d+).*input rate (\d+).*output rate (\d+)`)
	...
	for _, i := range interfaces {
		go func(i string) {
			...
			iface := match[1]
			bw, err := strconv.ParseFloat(match[2], 1)
			check(err)
			bw = bw * 1000
			input, err := strconv.ParseFloat(match[3], 1)
			check(err)
			output, err := strconv.ParseFloat(match[4], 1)
			check(err)
			...
			if in > 85.0 {
				fmt.Printf("warning - congestion detected on %v: %v\\%\n", iface, in)
			}
			if out > 85.0 {
				fmt.Printf("warning - congestion detected on: %v: %v\\%\n", iface, out)
			}
